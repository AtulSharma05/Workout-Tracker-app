...
if ($mongoRunning) {
    ...
    Write-Host "   ⚠️  MongoDB not detected - make sure it's running!" -ForegroundColor Yellow
}
Write-Host ""

# 2. Start Node.js Backend (Port 3000)
Write-Host "2️⃣  Starting Node.js Backend..." -ForegroundColor Yellow
cd backend
if (Test-Port 3000) {
    Write-Host "   ⚠️  Port 3000 already in use - killing existing process" -ForegroundColor Yellow
    Get-Process -Name node -ErrorAction SilentlyContinue | Stop-Process -Force
    Start-Sleep 2
}

Write-Host "   🚀 Launching Node.js server on port 3000..." -ForegroundColor Cyan
...
Write-Host "3️⃣  Starting AI Workout Planner..." -ForegroundColor Yellow
cd ai-planner
...
Write-Host "   🚀 Launching AI Planner on port 8000..." -ForegroundColor Cyan
...
if (Test-Port 8000) {
    Write-Host "   ✅ AI Planner running" -ForegroundColor Green
} else {
    Write-Host "   ❌ Failed to start AI Planner" -ForegroundColor Red
}
Write-Host ""

# 4. Pose Corrector (Python 3.10 Required, Port 8001)
Write-Host "4️⃣  Pose Corrector API..." -ForegroundColor Yellow

cd ..\pose-corrector

if (Test-Port 8001) {
    Write-Host "   ⚠️  Port 8001 already in use - skipping" -ForegroundColor Yellow
} else {
    ...
    if ($python310 -match "Python 3.10") {
        Write-Host "   ✅ Python 3.10 detected" -ForegroundColor Green
        
        # Check if venv exists
        if (Test-Path "venv-py310") {
            Write-Host "   🚀 Launching Pose Corrector on port 8001..." -ForegroundColor Cyan
            $posePath = (Get-Location).Path
            Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$posePath'; .\start_pose_api.ps1"
            Start-Sleep 4
            
            if (Test-Port 8001) {
                Write-Host "   ✅ Pose Corrector running" -ForegroundColor Green
            } else {
                Write-Host "   ⚠️  Pose Corrector starting (check the window)" -ForegroundColor Yellow
            }
        } else {
        ...
        Write-Host "   ❌ Python 3.10 not installed" -ForegroundColor Red
...
if (Test-Port 8001) {
    ...
    Write-Host "⏳ Pose Corrector:     Not running (requires Python 3.10)" -ForegroundColor Yellow
}
